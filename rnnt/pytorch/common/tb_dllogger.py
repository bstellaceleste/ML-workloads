...
def init_log(args):
    ...
    log_parameters(vars(args), tb_subset=None)
...
def log_parameters(data, verbosity=0, tb_subset=None):
    ...
    if tb_subset is not None and tb_loggers[tb_subset].enabled:
        ...
        #print("toto:", tb_data)
        keys = [0]
        tb_metric = {int(key):0 for key in keys}
        print("toto:", tb_metric)
        tb_loggers[tb_subset].summary_writer.add_hparams(tb_data, tb_data)
