...
def normalize_string(s, charset, punct_map):
    ...
    """
    #print("Speedyloader: normalize_string")
    charset = set(charset)
    try:
...
class DaliRnntIterator(object):
    ...
    def __init__(self, dali_pipelines, transcripts, tokenizer, batch_size, shard_size, pipeline_type, output_files, normalize_transcripts=False):
        print("Speedyloader: DaliRnntIterator.__init__")
        self.normalize_transcripts = normalize_transcripts
        self.tokenizer = tokenizer
        self.batch_size = batch_size
        self.of = output_files
    ...
    def tokenize(self, transcripts):
        print("Speedyloader: DaliRnntIterator.tokenize")
        transcripts = [transcripts[i] for i in range(len(transcripts))]
        if self.normalize_transcripts:
            print("Speedyloader: DaliRnntIterator.tokenize.transcript")
            transcripts = [
                normalize_string(
                    t,
            ...
            #for t in transcripts:
                ...
                #    punctuation_map(self.tokenizer.charset)
                #)
                #print("\nSpeedyloader-normalize_tr- time=", (time.time()-start_time)*1000," len(tr)=", len(t))
    ...
    def _gen_transcripts(self, labels, normalize_transcripts: bool = True):
        ...
        """
        #print("Speedyloader: DaliRnntIterator._gen_transcripts")
    ...
    def __next__(self):
        ...
        audio, audio_shape, label = data[0]["audio"], data[0]["audio_shape"][:, 1], data[0]["label"] #audio is an array of a single element, so data == data[0]; audio_shape seems to be the duration (because in train.py batch returns audio, audio_lens, text, and text_lens
        #tmp = label.item()
        #print("Speedyloader: label=", tmp)
        #for k,v in self.of.items():
        #    if tmp in v:
        #        print("Speedyloader: duration=", v[tmp])
