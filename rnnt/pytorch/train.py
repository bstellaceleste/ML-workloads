...
def main():
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch in train_loader:
            #n = n + 1
            #print("\nSpeedyloader: epoch",epoch, "batch",n)
            ...
            #logging.log_event(logging.constants.TRAIN_SAMPLES, value='Speedyloader', metadata={'audio_lenght':audio_lens})
#            log((step,), step, "Speedyloader", {'batch':batch, 'audio_lenght':audio_lens})

            #bef_feats = time.time()
            #duration = audio_lens.item()
            feats, feat_lens = train_feat_proc([audio, audio_lens])

            #print("Speedyloader: features_time = ",(time.time() - bef_feats) * 1000, "audio_lens = ", audio_lens[0].item(), "txt_lens = ", txt_lens[0].item()) #, "audio = ", audio[0].values())

            all_feat_lens += feat_lens

            #log((step,), step, "Speedyloader", {'feats_time':af_feats})
